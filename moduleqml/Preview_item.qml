...
Item {
	...
	Rectangle //主要覆盖层
	{
		...
		Rectangle {
			...
			color: "transparent"
			opacity: preview_item.state !== "default" ? 1 : textmodule.text.length > 0 ? 0.4 : 1
		...
		Rectangle {
			...
			color: "transparent"
			opacity: preview_item.state !== "default" ? 0 : 1
		...
		Rectangle //按钮域块
		{
			...
			Rectangle {
				...
				MouseArea {
					...
					onEntered: {
						//防止动画冲突
						enterAnim.running = preview_item.state === "default"
					}
					onClicked: {
						...
						//控制状态机
						preview_item.state = preview_item.state === "default" ? "Click" : "default"
					}
				}
			}
		}
			...
			Rectangle {
				...
				MouseArea {
					...
					Loader {
						...
						anchors.fill: parent
						source: {
							switch (preview_item.itemDirection) {
							case 0:
								switch (preview_item.itemIndex) {
								case 0:
									return "qrc:/pageqml/HotendModule.qml"
								case 1:
									return "qrc:/pageqml/HeatBedModule.qml"
								case 2:
									return "qrc:/pageqml/FanModule.qml"
								case 3:
									return "qrc:/pageqml/ProgressModule.qml"
								default:
									return undefined
								}
							case 1:
								switch (preview_item.itemIndex) {
								case 0:
									return "qrc:/pageqml/SettingsModule.qml"
								case 1:
									return "qrc:/pageqml/NetworkModule.qml"
								case 2:
									return "qrc:/pageqml/PowerModule.qml"
								case 3:
									return "qrc:/pageqml/CameraModule.qml"
								default:
									return undefined
								}
							default:
	...
	states: [
		State {
			...
			PropertyChanges {
				target: edge_btn
				width: preview_item.width - edge_btn_area.width - edge_btn_area.height * 0.1
			}
			//最终体积位置
			PropertyChanges {
				target: rectExpend
				width: preview_item.width - edge_btn_area.width * 2
				height: preview_item.height - 2 * (edge_btn_area.height * 0.1)
